...
type SidebarTab = "info" | "files" | "chat";

/** Resolve data file URL — in static mode use relative paths; in demo mode use env var URLs; otherwise use local paths with token. */
function dataUrl(fileName: string, token: string | null): string {
  if (STATIC_MODE) return `./${fileName}`;
  if (DEMO_MODE) {
 ...
 * In static mode, skip token entirely — data is public.
 */
function resolveInitialToken(): string | null {
  if (STATIC_MODE) return "__static__";
...
function App() {
  ...
  // In static or demo mode, skip token gate entirely
  if (STATIC_MODE || DEMO_MODE) {
    return <Dashboard accessToken={STATIC_MODE ? "__static__" : "__demo__"} />;
  }
...
function Dashboard({ accessToken }: { accessToken: string }) {
  ...
  const setSourceContent = useDashboardStore((s) => s.setSourceContent);
  ...
  useEffect(() => {
    // In static mode, load embedded source content for the CodeViewer
    if (accessToken !== "__static__") return;
    fetch(dataUrl("source-content.json", accessToken))
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (data) setSourceContent(data);
      })
      .catch(() => {});
  }, [accessToken, setSourceContent]);
    ...
    <div className="h-full flex flex-col min-h-0">
      <div className="flex items-center gap-1 p-2 border-b border-border-subtle bg-surface shrink-0">
        {(["info", "files", "chat"] as const).map((tab) => (
          ...
          >
            {tab === "info" ? t.sidebar.info : tab === "files" ? t.sidebar.files : "Chat"}
          </button>
        ))}
      </div>
      <div className="flex-1 min-h-0 overflow-auto">
        {sidebarTab === "files" ? (
          ...
          <Suspense fallback={<div className="p-4 text-sm text-text-muted">Loading chat...</div>}>
            <ChatPanel />
          </Suspense>
        ) : (
          infoSidebarContent
