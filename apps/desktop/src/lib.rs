...
/// 文件树里一个目录项：name + 绝对路径 + 是否目录。供 UI 懒加载渲染。
#[derive(Debug, Clone, serde::Serialize)]
struct DirEntry {
    name: String,
    path: String,
...
/// 列一个目录的直接子项（非递归）。dir-first，再按名字排序；隐藏项（点开头）排最后。
///
/// 供文件树面板按需展开用，不递归——展开一层拉一层，避免大仓库一次性遍历。
#[tauri::command]
fn read_dir(path: PathBuf) -> AppResult<Vec<DirEntry>> {
    let meta = std::fs::metadata(&path)?;
    if !meta.is_dir() {
...
/// 把编辑器内容写回磁盘文件。仅覆盖已存在的常规文件——拒绝新建 / 目录，
/// 避免文件查看器变成任意写入入口（创建文件应走 agent 的工具链）。
#[tauri::command]
fn write_text_file(path: PathBuf, content: String) -> AppResult<()> {
    let meta = std::fs::metadata(&path)?;
    if !meta.is_file() {
        return Err(AppError::msg("not a regular file"));
    }
...
fn empty_assistant_message() -> Message {
    use agent_core::storage::sessions::{self, Role};
    Message {
        id: sessions::new_id(),
        role: Role::Assistant,
        content: String::new(),
...
fn inject_user_message(
    ...
    let injected = if let Some(session_id) = common::runtime::session_for_request(&request_id) {
        ...
        cancellation::inject_pending_input(
            &request_id,
...
fn set_active_goal(app: AppHandle, session_id: String, condition: String) -> AppResult<()> {
    let dd = data_dir(&app)?;
    // 只写 active_goal（带 pending_set_marker），「目标已设」marker 由 agent_loop 在 run
    // 启动时（Goal set user 消息已落盘后）落——保证 marker 物理排在 user 消息之后，且与
    // 裁决 marker 同走 agent_core 串行落盘流，不靠前端抢落造成倒挂。
