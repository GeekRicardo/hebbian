...
### 2026-06-16 — 主对话输入框换成 Lexical 富文本，拖入 / @ / 粘贴路径统一成可删除的引用 chip

- **Why**: 用户拖一个不支持的文件（如 PDF）进输入框，原实现把它当「路径引用」塞进底部 `allowed_paths` chip 行——但那行默认折叠（要 hover 才展开），拖进去屏幕上没任何可见变化，用户以为「拖拽没生效」。用户的诉求是：引用要像别的 agent 里 `@文件` 那样，在输入框里**就地显示成一个高亮小块、可整块删除**。原 `@` 引用也只是把路径当纯文本插进 `<textarea>`，没有 chip。纯 textarea 物理上无法在文字流里内嵌渲染块，必须上 contenteditable。
- **改动**:
  - 后端 [apps/desktop/src/lib.rs](../apps/desktop/src/lib.rs)：抽 `normalize_dropped_path` 共享 file:// 解析（`attach_path` 复用）；新增 `drop_paths(paths) -> Vec<DropOutcome>` command + `classify_dropped_path`，按磁盘路径分流——图片(≤12MB)/文本(白名单,≤1MB)读成附件，目录/超限/二进制/未知→`Reference`，不存在→`Missing`。`DropOutcome` 的 serde tag/字段刻意对齐前端 `MessageAttachment`。配套单测 `drop_paths_classifies_by_type`（5 路分流）。
  - 前端 bridge [tauri.ts](../apps/desktop/frontend/src/desktop/bridge/tauri.ts)：新增 `dropPaths`，联合体类型与后端对齐。
  - **删除旧 [ChatInput.tsx](../apps/desktop/frontend/src/desktop/ui/components/ChatInput.tsx)（1464 行单文件）**，重写为 [chatInput/](../apps/desktop/frontend/src/desktop/ui/components/chatInput/) 目录（Lexical 富文本）：`ReferenceNode`（DecoratorNode 渲染 `@文件名` 高亮小块、可整块删）、`serialize.ts`（EditorState → `{text, references}`，chip 还原成路径原文 + 收集引用路径）、`editorState.ts`（`$setPlainText`/`$appendText`/`$insertReference` 纯 `$` 操作）、`editorConfig.ts`、`EditorSurface.tsx`（组合各插件 + 命令式 controller）、`index.tsx`（外壳：工具栏 / 底部 chip 行 / 抽屉 / submit-enqueue-cancel / 拖拽 / 附件）。plugins：`EditorCorePlugin`（Enter/方向键命令）、`SlashMenuPlugin`（`//` 联想）、`MentionMenuPlugin`（`@` 引用，复用 `ConversationRefPopup`）、`PastePlugin`（粘贴分流）、`useTriggerQuery`（typeahead 触发检测读 Lexical selection）。
  - [ChatView.tsx](../apps/desktop/frontend/src/desktop/ui/components/ChatView.tsx)：import 指向新目录。
  - [docs/架构.md](架构.md)：新增 §8.7 主对话输入框（Lexical 富文本 + 引用 chip），含节点模型 / 序列化发送链路 / 三套引用入口统一 / `drop_paths` 后端契约 / 程序性 update tag 防历史导航回归。
